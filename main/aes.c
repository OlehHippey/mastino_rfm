 ...
 */
int AES_set_encrypt_key(uint8_t *userKey, const int bits, AES_KEY *key) {
 ...
 */
int AES_set_decrypt_key(uint8_t *userKey, const int bits, AES_KEY *key) {
...
static AES_KEY	aesdkey;
static AES_KEY	aesekey;

void mastino_aes_set_key(uint8_t *key)
{
  AES_set_decrypt_key(key, 128, &aesdkey);
  AES_set_encrypt_key(key, 128, &aesekey);
}

void myaes_decrypt(uchar *buf, int n)
{
  int i;
  for(i=0; i<n; i+=16) AES_decrypt(buf+i, buf+i, &aesdkey);
}

 void myaes_encrypt(uchar *buf, int n)
{
   int i;
   for(i=0; i<n; i+=16) AES_encrypt(buf+i, buf+i, &aesekey);
}
